...
- **News:** asymmetric two-column cover grid — full-bleed duotone splash story on the left (Anton headline overlaid on the image), a sidebar on the right stacking notification opt-in and matchday banner. Below the cover, remaining articles run as a plain hairline-divided list. News stays news-only — no predictions teaser or games-coming-up block; those live on Home instead. Collapses to one column under 780px; the hero drops its absolute-positioned overlay and flows the headline below the image under 480px (a fixed-height hero with overlaid text broke on mobile once when the image was short and the headline long — don't reintroduce the overlay below 480px).
...
- Sponsor placements (header/feed/sidebar) are a confirmed real monetization mechanic, not filler — but switched off for the beta (`SPONSORS_ENABLED` in `src/lib/featureFlags.js`) so testers see a clean, unbranded product. Re-enable post-beta rather than treat their absence as the new direction.
- The "your stats" card family (`.stat-card-theme`, used on the Home picks-teaser, MyTeamDashboard, PredictionsSnapshot) previously ran its own champagne-gold palette (`#c9a35c` etc.) distinct from the rest of the site. That's gone — it now maps onto the real `--bg`/`--surface`/`--gold`/`--text` tokens so it reads as one visual system, not a second app skin.
